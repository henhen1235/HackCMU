...
from echoarena.tactics.reflex import AgentBody, ThreatShot, cardinal_clearances
...
def compose_snapshot(
    ...
    wall_dists = cardinal_clearances(
        AgentBody(x=bot.x, y=bot.y), walls, SCREEN_W, SCREEN_H
    )
    return {
        "bot": {
...
def play_round(
    ...
    # Dedicated arena surface — metrics live in a separate dashboard column
    ...
    def reset(next_layout: Optional[ArenaLayout] = None) -> None:
        ...
        nonlocal walls, current_layout, memory_saved, note_feed
    ...
    while not stop_flag.is_set():
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                ...
                if over and event.key in (pygame.K_r, pygame.K_RETURN, pygame.K_SPACE):
                    # Handled in game-over waiter; ignore here
                    pass
                if not over and event.key == pygame.K_1 and map_idx != 0:
                    map_idx = 0
                    reset(ARENAS[0])
                    _intro_curtain(screen, arena, ARENAS[0], 1.0, metrics)
                elif not over and event.key == pygame.K_2 and map_idx != 1:
                    map_idx = 1
                    reset(ARENAS[1])
                    _intro_curtain(screen, arena, ARENAS[1], 1.0, metrics)
                elif not over and event.key == pygame.K_3 and map_idx != 2:
                    map_idx = 2
                    reset(ARENAS[2])
        ...
        if over:
            if not memory_saved:
                ...
                memory_saved = True
            # Game-over choice still listens on the window
            outcome = _await_game_over_choice(screen, winner, arena, metrics)
            break

        # Mouse coords are window-relative; only aim inside the arena pane
        ...
        if (pygame.mouse.get_pressed()[0] or keys[pygame.K_SPACE]) and human.ready_to_fire():
            ...
            if shot:
                ...
                sparks += muzzle_sparks(human.x, human.y, shot.vx, shot.vy, PALETTE["shot_human"])
        ...
        METRICS.record_control_frame(
            used_llm=used_llm, under_threat=under_threat, dodging=dodging
        )

        if rfire and bot.ready_to_fire():
            shot = bot.shoot_at(lead_x, lead_y, "bot", cooldown=difficulty.bot_cooldown)
            if shot:
                shots.append(shot)
                METRICS.record_bot_shot()
                sparks += muzzle_sparks(bot.x, bot.y, shot.vx, shot.vy, PALETTE["shot_bot"])
        ...
        if note and note not in note_feed:
            for line in note.split("\n"):
                line = line.strip()
                if line:
                    note_feed.append(line)
                    bridge.push_note(line)
        ...
        METRICS.set_intent(
            age_s=intent_age if has_intent else 999.0,
            mx=ai_dx,
            my=ai_dy,
            fire=ai_fire,
            has_intent=has_intent,
